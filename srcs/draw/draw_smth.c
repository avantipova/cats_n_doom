...
/*   Updated: 2021/04/24 22:09:23 by npetrell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "duke.h"

static void	draw_menu_2(t_doom *doom, SDL_Rect rect, int step)
{
	int			i;
	int			j;

	if (doom->difficulty == 0)
		SDL_BlitScaled(doom->menu.difficulty_1, &rect, \
											doom->mgl->screen_surface, NULL);
	else if (doom->difficulty == 1)
		SDL_BlitScaled(doom->menu.difficulty_2, &rect, \
											doom->mgl->screen_surface, NULL);
	else
		SDL_BlitScaled(doom->menu.difficulty_3, &rect, \
											doom->mgl->screen_surface, NULL);
	rect.y -= step;
	...
	while (++i < 24)
	{
		...
		while (++j < 24)
		{
			put_pixel(doom->mgl->screen_surface->pixels, i - \
			W_2 + 50, -j - (doom->menu.active + 1.5) * step + H_2, 0xcea322);
		}
	}
}
