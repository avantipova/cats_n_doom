...
/*   Updated: 2021/04/24 20:07:23 by npetrell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	animation_update(t_scene *scene, float curr_time)
{
	...
	while (i < scene->sprites_count)
	{
		if (!scene->sprites[i].instance.model.anim || \
							!scene->sprites[i].instance.model.anim->played)
		{
			i++;
			continue ;
		}
		scene->sprites[i].instance.model.anim->curr_f += \
				scene->sprites[i].instance.model.anim->speed * curr_time;
